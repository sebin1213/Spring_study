...
//조회 V1: 안 좋은 버전, 모든 엔티티가 노출, @JsonIgnore -> 이건 정말 최악, api가 이거 하나인가! 화면에 종속적이지 마라!
    ...
    public Result membersV2() {
        ...
        List<MemberDto> collect = findMembers.stream()
                .map(m -> new MemberDto(m.getName())) // 멤버엔티티에서 이름을 꺼네와서 DTO에 넣고 MAP을하면 바꿔치는거임
                .collect(Collectors.toList());
        return new Result(collect.size(), collect); //감싸줘야함 리스트를 바로 내보내면 []같은 JSON배열타입으로 나가기때문에 유연성이 떨어짐(향후에 필드를 추가할수 없는 문제)
    }
    @Data
    @AllArgsConstructor
    static class Result<T> {
        private int count;
        private T data; //제네릭 T타입 데이터를 만들어줌
    }
    @Data
    @AllArgsConstructor
    static class MemberDto { //이름만 반환하는 DTO
        private String name;
    }
